...
unsigned int func_der_size=0;


std::vector<Xyce::Linear::Matrix*> Jac_v_test;
namespace Xyce {
namespace Analysis {
...
//  Linear::Matrix* p=test_p->getJacobianMatrix();
//   printf("%d@@@@@\n",p);
//  for(int i=0;i<p->getNumRows();++i){
//      for(int j=0;j<p->getRowLength(i);++j){
//         std::cout<< *(p->returnRawEntryPointer(i,j))<<"  ";
//      }
//      std::cout<<"\n";
//  }
...
//-----------------------------------------------------------------------------
// Function      : bool Transient::save_infor_for_sense
// Purpose       :
//
// Special Notes :
//
...
bool Transient::save_infor_for_sense(){
...
bool Transient::saveTransientAdjointSensitivityInfo ()
{
  ...
  printf("go to saveTransientAdjointSensitivityInfo ()\n");
  TimeIntg::DataStore & ds = * ( analysisManager_.getDataStore() );


//  cout<<"the ds1 pos is "<<&ds<<"\n";
...
//  ds.test_v.push_back(currentTimeStep);
//  cout<<"test_v size is "<<ds.test_v.size()<<"\n";
  // save the time point:
  ...
  int s_size=0;
  for(int i=0;i<ds.solutionHistory.size();++i){
      s_size=s_size+ds.solutionHistory[i]->get_size();
  }
  solution_size=0;
  solution_size=s_size;

  ds.stateHistory.push_back(ds.nextStatePtr->cloneCopyVector());


  int sta_size=0;
  for(int i=0;i<ds.stateHistory.size();++i){
      sta_size=sta_size+ds.stateHistory[i]->get_size();
  }
  state_size=0;
  state_size=sta_size;
  ...
  int stor_size=0;
  for(int i=0;i<ds.storeHistory.size();++i){
      stor_size=stor_size+ds.storeHistory[i]->get_size();
  }
  store_size=0;
  store_size=stor_size;





//  std::cout<<"the pos of store 0 is "<<&ds.storeHistory<<"\n";
  cout<<"the size of solutionHistory is "<<ds.get_solutionHistory_size()<<"\n";

  if (!newLowMem_)// 一般情况下 newLowMem_==0 
  {
...
//    std::cout<<"the value of sparseAdjointStorage_ is "<<sparseAdjointStorage_<<"\n";
...
//      (ds.JMatrixPtr);
      // adjoint sparse storage experiment
       ...
       cout<<"the size of sparseFunctionSensitivity is "<<ds.get_sparseFunctionSensitivity_size()<<"\n";
//       cout<<"the size of sparseRHS is "<<sizeof (*ds.sparseSensRHSMV)<<"\n";
//       cout<<"the size of sparseJ is "<<sizeof (*ds.JMatrixPtr)<<"\n";
//       cout<<"the size of ds is "<<sizeof (ds)<<"\n";
//       cout<<"the pos of ds is "<<&ds<<"\n";
...
//      int f_size=0;
//      for(int i=0;i<ds.functionSensitivityHistory.size();++i){
//          f_size=f_size+ds.functionSensitivityHistory[i]->get_size();

//      }



//      cout<<"the size of RHS is "<<sizeof (*ds.sensRHSPtrVector)<<"\n";
//       cout<<"the size of J is "<<sizeof (*ds.JMatrixPtr)<<"\n";
    }
//    print_history(ds.storeHistory);
  }
...
/*
  ...
  string temp_str;
//        cout<<"\n";
        for(int i=0;i<ds.dQdxMatrixPtr->getNumRows();++i){
            for(int j=0;j<ds.dQdxMatrixPtr->getRowLength(i);++j){
//               cout<< *(ds.dQdxMatrixPtr->returnRawEntryPointer(i,j))<<"  ";
               temp_str=temp_str+std::to_string(*(ds.dQdxMatrixPtr->returnRawEntryPointer(i,j)))+"  ";
            }
//            cout<<"\n";
            temp_str=temp_str+"\n";
        }
...
//        cout<<"one step back"<<"\n";
//        cout<<"\n";
        write_file(temp_str,"./dQdxinfo.txt",1);
        temp_str="";


        for(int i=0;i<ds.dFdxMatrixPtr->getNumRows();++i){
            for(int j=0;j<ds.dFdxMatrixPtr->getRowLength(i);++j){
//               cout<< *(ds.dFdxMatrixPtr->returnRawEntryPointer(i,j))<<"  ";
               temp_str=temp_str+std::to_string(*(ds.dFdxMatrixPtr->returnRawEntryPointer(i,j)))+"  ";
            }
//            cout<<"\n";
            temp_str=temp_str+"\n";
        }
        temp_str=temp_str+"\n";
//        cout<<"two steps back\n";
        write_file(temp_str,"./dFdxinfo.txt",1);
        temp_str="";
*/
/*
  string temp_str;
//        cout<<"\n";
        for(int i=0;i<ds.JMatrixPtr->getNumRows();++i){
            for(int j=0;j<ds.JMatrixPtr->getRowLength(i);++j){
//               cout<< *(ds.dQdxMatrixPtr->returnRawEntryPointer(i,j))<<"  ";
               temp_str=temp_str+std::to_string(*(ds.JMatrixPtr->returnRawEntryPointer(i,j)))+"  ";
            }
//            cout<<"\n";
            temp_str=temp_str+"\n";
        }
...

{
      string temp_str;
        for(int i=0;i<ds.dFdxMatrixPtr->getNumRows();++i){
            for(int j=0;j<ds.dFdxMatrixPtr->getRowLength(i);++j){
      //           cout<< *(ds.dFdxMatrixPtr->returnRawEntryPointer(i,j))<<"  ";
               temp_str=temp_str+d2s(*(ds.dFdxMatrixPtr->returnRawEntryPointer(i,j)))+"  ";
            }
      //        cout<<"\n";
            temp_str=temp_str+"\n";
        }
        temp_str=temp_str+"\n";
      //    cout<<"damp two steps back\n";
        write_file(temp_str,"./dFdxTrans1.txt",1);
        temp_str="";
...
}
  {
      ...
      for(int i=0;i<ds.dQdxMatrixPtr->getNumRows();++i){
          for(int j=0;j<ds.dQdxMatrixPtr->getRowLength(i);++j){
    //           cout<< *(ds.dFdxMatrixPtr->returnRawEntryPointer(i,j))<<"  ";
             temp_str=temp_str+d2s(*(ds.dQdxMatrixPtr->returnRawEntryPointer(i,j)))+"  ";
          }
    //        cout<<"\n";
          temp_str=temp_str+"\n";
      }
      temp_str=temp_str+"\n";
    //    cout<<"damp two steps back\n";
      write_file(temp_str,"./dQdxTrans1.txt",1);
      temp_str="";
...
//          for(int j=0;j<ds.JMatrixPtr->getRowLength(i);++j){
//    //           cout<< *(ds.dFdxMatrixPtr->returnRawEntryPointer(i,j))<<"  ";
//             temp_str=temp_str+d2s(*(ds.JMatrixPtr->returnRawEntryPointer(i,j)))+"  ";
//          }
//    //        cout<<"\n";
//          temp_str=temp_str+"\n";
//      }
//      temp_str=temp_str+"\n";
//    //    cout<<"damp two steps back\n";
//      write_file(temp_str,"./compare/JC.txt",1);
//      temp_str="";
//  }

  int dFdx_size=0;
  int dQdx_size=0;
  dFdx_size=dFdx_size+ ds.dFdxMatrixPtr->get_size();
  dQdx_size=dQdx_size+ ds.dQdxMatrixPtr->get_size();
  cout<<"the size of dFdx is "<< dFdx_size<<"\n";
  cout<<"the size of dFdx1 is "<<malloc_usable_size(ds.dFdxMatrixPtr)<<"\n";

  cout<<"the size of dQdx is "<< dQdx_size<<"\n";

  dQdx_s=dQdx_s+ds.get_dQdx_size();
  dFdx_s=dFdx_s+ds.get_dFdx_size();
  cout<<"compare 1 "<<ds.get_dQdx_size()<<"\n";
//  cout<<"dFdxMatrixPtr len len "<<malloc_usable_size(ds.dFdxMatrixPtr)/sizeof(ds.dFdxMatrixPtr[0])<<"\n";
  Xyce::Linear::EpetraMatrix* jac_temp=dynamic_cast<Xyce::Linear::EpetraMatrix*>(ds.JMatrixPtr);


//  Linear::EpetraMatrix* m_new=new Linear::EpetraMatrix(&(jac_temp->epetraObj()),false);
  //复制matrix尝试
  /*
  Linear::EpetraMatrix* m_new=new Linear::EpetraMatrix(jac_temp->getOverlapGraph(),jac_temp->getGraph());
  m_new->add(*(ds.JMatrixPtr));
  m_new->addOverlap(*(ds.JMatrixPtr));
  Linear::Matrix* m_new_temp=dynamic_cast<Xyce::Linear::Matrix*>(m_new);
  m_new_temp->print(Xyce::dout());

  ds.Jac_v.push_back(m_new_temp);
  */


//  Jac_v_test.push_back(m_new_temp);

  if(fast_sens){
//       ds.construct_tensor(ds.JMatrixPtr->clone_m(),ds.Jac_v);

//       ds.construct_tensor(ds.dQdxMatrixPtr->clone_m(),ds.dqdx_v);

//       ds.construct_tensor(ds.dFdxMatrixPtr->clone_m(),ds.dfdx_v);

//      clock_t start=clock();

       ds.Jac_v.push_back(ds.JMatrixPtr->clone_m());
//       ds.dqdx_v.push_back(ds.dQdxMatrixPtr->clone_m());
//       ds.dfdx_v.push_back(ds.dFdxMatrixPtr->clone_m());

       ds.rhs_v.push_back(ds.RHSVectorPtr->cloneCopyVector());

//       clock_t end=clock();
//       double elapsed = double(end - start)/CLOCKS_PER_SEC;
//       reduce_time=reduce_time-elapsed;

  }else{
       ds.evaluate_memo_cos_virtual(ds.JMatrixPtr->clone_m(),is_bitmap);
...
//      output_count++;
//      ofstream ofs;
////      ofs.open("./m"+std::to_string(output_count), ios::out);

////      ds.JMatrixPtr->print(ofs);

////      ofs.open("./"+std::to_string(output_count),ios::out);
//      ofs.open("./output0", ios::app);
////      ds.JMatrixPtr->print_m(ofs);
//       ds.JMatrixPtr->print_value(ofs);
//      ofs.close();

//  }

//ds.JMatrixPtr->print(Xyce::dout());
  return true;
}
...
//此处保存瞬态过程中的数据，若想利用，更改virtual属性，并更改之后的sens文件
//       clock_t start=clock();
...
//doTransientAdjointSensitivity->calcTransientAdjoint->solveTransientAdjoint
...
//10.28
//  for(int i=0;i<ds.Jac_v.size();++i){
// //      ds.Jac_v[i].print(Xyce::dout());
// //    cout<<ds.Jac_v[i]<<"\n";


//    string temp_str;
//    for(int ii=0;ii<ds.Jac_v[i]->getNumRows();++ii){
//        for(int j=0;j<ds.Jac_v[i]->getRowLength(ii);++j){
//  //           cout<< *(ds.dFdxMatrixPtr->returnRawEntryPointer(i,j))<<"  ";
//           temp_str=temp_str+d2s(*(ds.Jac_v[i]->returnRawEntryPointer(ii,j)))+"  ";
//        }
//  //        cout<<"\n";
//        temp_str=temp_str+"\n";
//    }
//    temp_str=temp_str+"\n";
//  //    cout<<"damp two steps back\n";
//    write_file(temp_str,"./compare/JC_save.txt",1);
//    temp_str="";
...
if(fast_sens){
    // cout<<"Jac_v size before is "<<ds.Jac_v.size()<<"\n";

//     memo_infor=memo_infor+"the number of matrix after compress is "+ l2s(ds.Jac_v.size(),1)+"\n";

    // cout<<"Jac_v memo_cos before is "<<ds.evaluate_memo_cos(0)<<"\n";


//     memo_infor=memo_infor+"the size of tensor after compress is "+ l2s(ds.evaluate_memo_cos(1)/1024,1)+"KB\n";


//     ds.decompress_tensor(ds.Jac_v);

//     ds.decompress_tensor(ds.dqdx_v);

//     ds.decompress_tensor(ds.dfdx_v);
    // cout<<"Jac_v size after is "<<ds.Jac_v.size()<<"\n";
//     memo_infor=memo_infor+"the number of matrix before compress is "+ l2s(ds.Jac_v.size(),1)+"\n";
    // cout<<"Jac_v memo_cos after is "<<ds.evaluate_memo_cos(0)<<"\n";
//     memo_infor=memo_infor+"the size of tensor before compress is "+ l2s(ds.evaluate_memo_cos(0)/1024,1)+"KB\n";
}else{
    // cout<<"Jac_v size before is "<<ds.Jac_v.size()<<"\n";

//     memo_infor=memo_infor+"the number of matrix after compress is "+ l2s(ds.virtual_num,1)+"\n";

    // cout<<"Jac_v memo_cos before is "<<ds.evaluate_memo_cos(0)<<"\n";

//     memo_infor=memo_infor+"the size of tensor after compress is "+ l2s(ds.virtual_size/1024,1)+"KB\n";


    // cout<<"Jac_v size after is "<<ds.Jac_v.size()<<"\n";
//     memo_infor=memo_infor+"the number of matrix before compress is "+ l2s(ds.virtual_step,1)+"\n";
    // cout<<"Jac_v memo_cos after is "<<ds.evaluate_memo_cos(0)<<"\n";
//     memo_infor=memo_infor+"the size of tensor before compress is "+ l2s(ds.virtual_size_0/1024,1)+"KB\n";

}
// memo_infor=memo_infor+"\ndoTransientAdjointSensitivity\n";
// write_file(memo_infor,"./data.txt",1);



/*
  for(int i=0;i<Jac_v_test.size();++i){
//      ds.Jac_v[i].print(Xyce::dout());
//    cout<<ds.Jac_v[i]<<"\n";


    string temp_str;
    for(int ii=0;ii<Jac_v_test[i]->getNumRows();++ii){
        for(int j=0;j<Jac_v_test[i]->getRowLength(ii);++j){
  //           cout<< *(ds.dFdxMatrixPtr->returnRawEntryPointer(i,j))<<"  ";
           temp_str=temp_str+d2s(*(Jac_v_test[i]->returnRawEntryPointer(ii,j)))+"  ";
        }
  //        cout<<"\n";
        temp_str=temp_str+"\n";
    }
    temp_str=temp_str+"\n";
  //    cout<<"damp two steps back\n";
    write_file(temp_str,"./compare/JC_save_1.txt",1);
    temp_str="";
...
//  cout<<"test_v size is "<<ds.test_v.size()<<"\n";
  ...
  cout<<"the size of datastore is "<<sizeof (TimeIntg::DataStore)<<"\n";
  TimeIntg::StepErrorControl & sec = analysisManager_.getStepErrorControl();
  ...
  time_points=numTimePoints;
//  cout<<"the value of numTimePoints is "<<numTimePoints<<"\n";
...
//10.28
//      cout<<"the value of index size is "<<adjointTimePointIndices_.size()<<"\n";
//      string temp_str;
//    //        cout<<"\n";
//            for(int i=0;i<ds.JMatrixPtr->getNumRows();++i){
//                for(int j=0;j<ds.JMatrixPtr->getRowLength(i);++j){
...
//    //        cout<<"one step back"<<"\n";
//    //        cout<<"\n";
//            write_file(temp_str,"./JC_Sens.txt",1);
//            temp_str="";


//     cout<<"the value of iatpi is "<<iatpi<<"\n";
...
//           cout<<"one step back"<<"\n";
//           cout<<"\n";
           write_file(temp_str,"./dQdxSens1.txt",1);
           temp_str="";
...
//               cout<<"\n";
           ...
           temp_str=temp_str+"\n";
//           cout<<"two steps back\n";
           write_file(temp_str,"./dFdxSens1.txt",1);
           temp_str="";

*/
//计算敏感度时间点
    int itGlobal=adjointTimePointIndices_[iatpi];//0,1,2...
    ...
    int adjointBeginIndex = itGlobal;//0,1,2
...
//            temp_str=temp_str+"\n";
//          //    cout<<"damp two steps back\n";
//            write_file(temp_str,"./dQdxTrans2.txt",1);
//            temp_str="";
...
//            temp_str=temp_str+"\n";
//          //    cout<<"damp two steps back\n";
//            write_file(temp_str,"./dFdxSenseNew.txt",1);
//            temp_str="";


            dFdx_size=dFdx_size+ ds.dFdxMatrixPtr->get_size();
            dQdx_size=dQdx_size+ ds.dQdxMatrixPtr->get_size();
//      cout<<"the size of dFdx2 is "<<malloc_usable_size(ds.dFdxMatrixPtr)<<"\n";
...
//      cout<<"the size of ds_after is "<<sizeof (ds)<<"\n";
      if (it==0)
      {
        dcopFlag_ = true;
      }
...
//      ds.dfdpHistory;
  ...
  cout<<"the size of dfdx is "<< dFdx_size+dQdx_size<<"\n";


  string time_infor="";
  time_infor=time_infor+"compress time is: "+d2s(compress_time,1)+"\n";
  write_file(time_infor,"./data.txt",1);
...
bool Transient::finalVerboseOutput()
{
  ...
  if (analysisManager_.getAnalysisMode() == ANP_MODE_TRANSIENT)
  {
    ...
    time_infor=time_infor+" ***** DCOP time: "+d2s(time,1)+" seconds.  Breakdown follows:\n";
  ...
  if (analysisManager_.getAnalysisMode() == ANP_MODE_TRANSIENT && endTRANtime_ >= startTRANtime_)
  {
    ...
    time_infor=time_infor+" ***** Transient Stepping time: "+d2s(endTRANtime_ - startTRANtime_,1)+" seconds.  Breakdown follows:\n";
...
void Transient::takeAnIntegrationStep_()
{
  ...
  printf("go to takeAnIntegrationStep_\n");
...
void Transient::print_history(vector<Linear::Vector*> v){
